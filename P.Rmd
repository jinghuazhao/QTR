...
names(s)
```
Consider now the `Subject` effect. From
...
names(s)
```
We saw the same estimate of effect but a larger standard error for `Days` in the linear mixed model compared to
...
```
Compared to its counterpart in `lme4`, `lmerTest` provides P value.
