...
systolic <- phen$systolic
diastolic <- phen$diastolic
depression_score <- phen$depression_score
age <- phen$age
...
d <- data_d[1, ]
CpG <- as.numeric(d)
lmer.summary <- summary(lmer(depression_score ~ CpG + age + gender + systolic + PCs$PC1 + PCs$PC2 + PCs$PC3 + PCs$PC4 + PCs$PC5 + (1|fid),
...
result <- rbindlist(alply(data_d, 1, function(obs) {
  tryCatch({
    CpG <- as.numeric(obs)
    s <- summary(lmer(depression_score~CpG + age + gender + systolic + PCs$PC1 + PCs$PC2 + PCs$PC3 + PCs$PC4 + PCs$PC5 + (1|fid),
                      data=phen))$coefficient
