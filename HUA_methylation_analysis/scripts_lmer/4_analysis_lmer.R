...
dummy <- summary(lmer(dummy ~ SUA + age + gender + scoretotal + BMI + systolic + GLU + CHOL + TG + HDLC + LDLC + PCs$PC1 + PCs$PC2 + PCs$PC3 + PCs$PC4 + PCs$PC5 + (1|fid) ))$coefficient
...
result <- rbindlist(alply(data_d, 1, function(obs) {
	tryCatch({
		sumt <- summary(lmer(as.numeric(obs) ~ SUA + age + gender + scoretotal + BMI + systolic + GLU + CHOL + TG + HDLC + LDLC + PCs$PC1 + PCs$PC2 + PCs$PC3 + PCs$PC4 + PCs$PC5 + (1|fid) ))$coefficient
