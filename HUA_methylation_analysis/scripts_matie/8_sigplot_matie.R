...
print(sigresult_tex, file = "rrbs_clean_data_matie/sigDMR.tex")
write.csv(sigresult, file = "rrbs_clean_data_matie/sigDMR.csv", row.names = FALSE)

resultname <- 'rrbs_clean_data_matie/lmer.RDS'
...
for(i in 1:nrow(sigresult)) {
  pdf(file = paste0('rrbs_clean_data_matie/DMR_', i, '.pdf'), width = 8.3, height = 4)
...
pdf(file = paste0("rrbs_clean_data_matie/DMRs.pdf"), width = 10, height = 16.5)
num_per_page <- 8
col_per_page <- 2
row_per_page <- 4
...
for(npage in 1:num_page) {
	...
	for(i in startplot:endplot) {
		...
		plot_list[[i-startplot+1]] <- ggplot(data = plotdata2, mapping = aes(y = `cognitive_function_score:Estimate`, x = start)) + geom_smooth() + geom_point() + xlab('BP') + ylab('Coefficient') + ggtitle(bquote(bold(.(LETTERS[i-startplot+1])) * ' DMR ' * .(plotdata1$V1) * ':' * .(plotdata1$V2) * '-' * .(plotdata1$V3))) + theme_bw() + scale_y_continuous(labels = fmt_dcimals(3)) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
	}
...
pdf(file = paste0('rrbs_clean_data_matie/DMRsr.pdf'), width = 15, height = 11.7)
num_per_page <- 9
col_per_page <- 3
row_per_page <- 3
num_page <- ceiling(nrow(sigresult)/num_per_page)
...
#   plotdata2 <- cpgresult[(cpgresult$seqnames == plotdata1$V1) & (cpgresult$start >= plotdata1$V2) & (cpgresult$end <= plotdata1$V3), ]
#   # plot_list[[i]] <- ggplot(data = plotdata2, mapping = aes(y = `cognitive_function_score:Estimate`, x = start)) + geom_smooth() + geom_point() + xlab('BP') + ylab('Coefficient') + ggtitle(paste0(LETTERS[i], ' DMR ', plotdata1$V1, ':', plotdata1$V2, '-', plotdata1$V3)) + theme_bw() + scale_y_continuous(labels = fmt_dcimals(2)) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
#   plot_list[[i]] <- ggplot(data = plotdata2, mapping = aes(y = `cognitive_function_score:Estimate`, x = start)) + geom_smooth() + geom_point() + xlab('BP') + ylab('Coefficient') + ggtitle(bquote('DMR ' * .(plotdata1$V1) * ':' * .(plotdata1$V2) * '-' * .(plotdata1$V3))) + theme_bw() + scale_y_continuous(labels = fmt_dcimals(2)) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
# }
# pdf(file = paste0('rrbs_clean_data_matie/DMRsr_ar.pdf'), width = 15, height = 11.7)
# figure <- ggarrange(plot_list[[1]], plot_list[[2]], plot_list[[3]], plot_list[[4]], plot_list[[5]], plot_list[[6]], plot_list[[7]], plot_list[[8]], plot_list[[9]], plot_list[[10]], plot_list[[11]], ncol = 3, labels = LETTERS[1:11])
