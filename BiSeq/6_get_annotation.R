...
getwd()
range_result_name <- 'rrbs_clean_data1/result_range.RDS'
...
#while using "id_type <- 'entrezgene'", the following script could not be performed
id_type <- 'entrezgene'
anno <- getBM(attributes = c(id_type, 'chromosome_name', 'start_position', 'end_position'), mart = ensembl)

#while using "id_type <- 'ensembl_gene_id'", then the following script could be performed.However, the results may be not consistent
#with the former
...
sigresult <- readRDS('rrbs_clean_data1/sigresult.RDS')
...
#because the "result_range_entrezgene.RDS" has certain problems, then I used the former results
anno <- readRDS('rrbs_clean_data/result_range_entrezgene.RDS')
sigresult <- sigresult[, c('unit', 'seqnames', 'start', 'cognitive_function_score:Pr(>|t|)', 'cognitive_function_score:Estimate', 'padj')]
colnames(sigresult) <- c('unit', 'Chromosome', 'Position', 'pvalue', 'effectsize', 'padj')
anno <- anno[, c('unit', "entrezgene")]
library(plyr)
sigresult <- join(sigresult, anno)
write.csv(sigresult, file = "rrbs_clean_data1/sigresult_anno_entrezgene.csv", row.names = FALSE)
